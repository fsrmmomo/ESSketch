...
#define TOT_MEM_IN_BYTES (300 * 1024)
...
//        int count = packet_cnt/slot;
//        int res = packet_cnt % slot;
//        int index = 0;
//        for (int i = 0; i < count; ++i){
//            for (int j = 0; j < slot; ++j){
//                string str((const char *) (traces[datafileCnt - 1][index].key), 4);
//                index++;
//                Aggregation_insert[str]++;
//            }
//            for(unordered_map<string,int>::iterator iter=Aggregation_insert.begin();iter!=Aggregation_insert.end();++iter){
//                uint8_t key[4];
//                memcpy(key, (iter->first).c_str(), 4);
//                elastic->insert(key,iter->second);
//            }
//            Aggregation_insert.clear();
//        }
//        for (int i = 0; i < res; ++i){
//            string str((const char *) (traces[datafileCnt - 1][index].key), 4);
//            index++;
...
//        Aggregation_insert.clear();
        ...
        printf("%d.dat: packet_cnt=%d\t", datafileCnt - 1, packet_cnt);
        printf("%d.dat: Real_Freq.size()=%d\t", datafileCnt - 1, (int) Real_Freq.size());
        ARE /= (int) Real_Freq.size();

        ////////////////////
        AREList[datafileCnt-1] = ARE;
        ///////////////////////////
...
//    outFileName.append(to_string(slot)).append("slot.res");
//    outFileName.append(to_string(slot)).append("slot_300KB.res");
//    ofstream outARE(outFileName,ios::trunc);
//    for (int i = 0; i < END_FILE_NO; ++i){
//        outARE << AREList[i]<<endl;
//    }
//    outARE.close();

//    string outFileName2 = "../result/";
//    outFileName2.append(to_string(slot)).append("slot_evict_300KB.res");
//    ofstream outEvict(outFileName2,ios::trunc);
//    outEvict<<count_evict<<endl;
//    printf("evict:%d", count_evict);
//    outEvict.close();
